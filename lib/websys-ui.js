    ...
    try {
      ...
      for (const d of devices) {
        ...
        div.innerHTML = '<strong>' + (d.name || '?') + '</strong>' +
          ' <button class="rename-device-btn" data-device-id="' + (d.vault_bee_writer || '') + '" style="font-size:10px;padding:0px 4px;margin-left:4px">Rename</button>' +
          (is_self ? ' <span style="font-size:10px;color:#007bff">(This Device)</span>' : '') + dead_indicator + '<br>' +
