...
function show_system_bar () {
  ...
  ***************************************/
    ...
    try {
      ...
      el.innerHTML = apps.map(function (app) {
        return '<div style="border:1px solid #ccc;padding:6px;margin:4px 0">' +
          '<strong>' + (app.name || app.id) + '</strong><br>' +
  ...
  async function handle_app_audit (app_id) {
    const content = sys.querySelector('#app-mgmt-' + app_id)
  ...
  async function handle_app_raw_struct (app_id, struct_name) {
    const content = sys.querySelector('#app-mgmt-' + app_id + ' .struct-content')
    if (!content) return
  ...
  async function handle_app_reset (app_id) {
    ...
    try {
      ...
      sys.querySelector('#app-mgmt-' + app_id).textContent = 'Data cleared.'
      load_apps_mgmt()
    } catch (err) { sys.querySelector('#app-mgmt-' + app_id).textContent = 'Error: ' + err.message }
  }
