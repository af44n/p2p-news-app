...
- Handles multi-device pairing with robust workflows (petnames, safe removal prompts)
- Sets up P2P networking (Hyperswarm, DHT relay, WebRTC)
- Offline-cached mode for graceful relay connection failures
- Tracks device liveness and logs device events
...
// Look at all device events
const events = await vault.get_events(events_drive)

// See who is paired
const devices = await vault.get_paired_devices(events_drive)

// Find out which devices have been offline forever
const inactive = await vault.get_inactive_devices(events_drive)

// Kick a device out (asks for confirmation first)
await vault.remove_device(events_drive, writer_key)

// Give devices cute names so you know what's what
await vault.set_device_petname(events_drive, writer_key, 'Work Laptop')
