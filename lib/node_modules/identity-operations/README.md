...
// Device Management
// listing devices, naming them, kicking them out, and checking if they're dead
...
// Kick a device (has a safety prompt now)
await vault.remove_device(events_drive, device)

// See which devices haven't pinged in a while
const inactive_devices = await vault.get_inactive_devices(events_drive)

// Give devices petnames and save your recovery keys from getting wiped
await vault.set_device_petname(events_drive, writer_key, 'My Phone')
await vault.set_recovery_exemption(events_drive, writer_key, true)
```
