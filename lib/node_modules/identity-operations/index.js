...
function create_vault_ops (get_state) {
  ...
  ***************************************/
  ...
  async function register_app (app_id, app_config) {
    ...
    if (!state.store) throw new Error('Network store not initialized')
    ...
    async function handle_app_registration () {
      const app_audit = create_auditcore({
        store: state.store.namespace(`app-audit-${app_id}`)
      })
  ...
  ***************************************/
  ...
  function start_writer_watcher (app_id) {
    ...
    if (!state.vault_bee) return
    if (app_id) {
      state.active_app_id = app_id
      const bg = state.background_managers.get(app_id)
      if (bg) {
        for (const name of bg.get_names()) {
          const inst = bg.get(name)
          if (inst?.close) inst.close().catch(() => {})
        }
        state.background_managers.delete(app_id)
      }
    }
    if (state.writer_watcher_active) return
    state.writer_watcher_active = true
    ...
    const watcher = state.vault_bee.watch({ gte: 'writer_requests/', lt: 'writer_requests0' })
    ...
    async function process_writer_requests () {
      try {
        ...
        for (const app of apps) {
          await process_writer_requests_for_app(app.id)
        }
      } catch (err) {
        console.error('[identity] Writer processing error:', err.message)
      }
    }
    async function process_writer_requests_for_app (app_id) {
      const requests = await vault_get(`writer_requests/${app_id}`)
      if (!requests) return
      const manager = await get_manager_for_app(app_id)
      let writable = false
      for (const name of manager.get_names()) {
        const structure = manager.get(name)
        if (structure?.writable || structure?.base?.writable) { writable = true; break }
      }
      if (!writable) return
      const our_key_hex = b4a.toString(state.vault_bee.base.local.key, 'hex')
      let processed_any = false
      for (const req of requests) {
        if (req.processed) continue
        if (req.vault_bee_writer === our_key_hex) continue 
        const added = await add_writers_from_request(app_id, req)
        if (!added) continue
        processed_any = true
        req.processed = true
  ...
  async function add_writers_from_request (app_id, req) {
    ...
    for (const [name, key] of Object.entries(req.writer_keys)) {
      try {
        if (name === 'app_audit') {
          let app_audit = state.app_audits.get(app_id)
          if (!app_audit) {
            const app = await get_app(app_id)
            if (app && app.audit_key) {
              app_audit = await load_app_audit(app_id, app.audit_key)
            }
          }
          if (app_audit) await app_audit.add_writer(key)
        } else {
          await manager.add_writer(name, key)
        }
  ...
  // lazy load the app based on request

  async function get_manager_for_app (app_id) {
    const state = get_state()
    if (app_id === state.active_app_id && state.ds_manager) return state.ds_manager
    if (state.background_managers.has(app_id)) return state.background_managers.get(app_id)

    const app = await get_app(app_id)
    const app_store = state.store.namespace(app_id)
    await app_store.ready()

    const manager = datastructure_manager()
    const keys_map = Object.fromEntries(app.structures.filter(s => s.key).map(s => [s.name, s.key]))
    await manager.init_all(app.structures.map(s => ({ ...s, store: app_store })), keys_map)

    state.background_managers.set(app_id, manager)
    return manager
  }
  ...
  async function log_bootstrap_device (app_id) {
    ...
    if (state.app_audits.get(app_id)?.base?.local) device_keys.app_audit_writer = b4a.toString(state.app_audits.get(app_id).base.local.key, 'hex')
