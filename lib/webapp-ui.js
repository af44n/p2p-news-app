...
async function render_view (view, ...args) {
  ...
  async function render_audit () {
    ...
    const devs = await uservault.get_paired_devices()
    const list = document.createElement('ul')
    logs.reverse().forEach(entry => {
      const { device_id, ...data } = entry.data || {}
      const dev = devs.find(d => d.app_audit_writer === device_id)
      const dev_name = dev ? dev.name : (device_id?.slice(0, 8) || '')
      const li = document.createElement('li')
      li.style.marginBottom = '20px'
      li.style.borderBottom = '1px solid #ccc'
      li.style.paddingBottom = '10px'
      li.innerHTML = `<strong>${entry.type}</strong>${dev_name ? ` <span style="color:#0066cc">[${escape_html(dev_name)}]</span>` : ''} <small>${new Date(data.timestamp).toLocaleString()}</small><br><pre style="background:#f4f4f4;padding:5px;margin-top:5px;">${JSON.stringify(data, null, 2)}</pre>`
      list.appendChild(li)
    })
